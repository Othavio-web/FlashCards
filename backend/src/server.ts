...
app.get('/api/cards/category/:category', async (req: Request, res: Response): Promise<void> => {
  try {
    const cards = await Card.findAll({
...
app.post('/api/cards', async (req: Request, res: Response): Promise<void> => {
  try {
...
// 📝 PUT: Editar um card
app.put('/api/cards/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const { category, question, answer } = req.body;
...
app.delete('/api/cards/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    const cardId = parseInt(req.params.id as string, 10);
